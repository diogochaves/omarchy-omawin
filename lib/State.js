...
function failure(sample, desired, now) {
  ...
  if (!sample || !sample.installed) return ""
  var action = desired && desired.action ? desired.action : null
  var since = desired && isFinite(desired.since) ? desired.since : 0
  var elapsed = (isFinite(now) ? now : 0) - since
  if (action === "start" && !sample.pid && elapsed >= START_TIMEOUT)
  ...
  if (action === "stop" && sample.pid && elapsed >= STOP_TIMEOUT)
...
function cacheFrom(sample, previous, now) {
  ...
  // A disk bigger than the one last written down has just been grown; a grow
  // already recorded is kept for as long as this same run lasts.
  var grew = null
  if (DISK_SHAPE.test(String(before.disk)) && DISK_SHAPE.test(String(next.disk))
    && parseInt(next.disk, 10) > parseInt(before.disk, 10))
    grew = { from: String(before.disk), to: next.disk }
  else if (next.started && Number(before.started) === next.started)
